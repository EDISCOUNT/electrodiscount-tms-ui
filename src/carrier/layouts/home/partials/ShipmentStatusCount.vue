<template>
    <!-- {{ {criteria} }} -->
    <!-- {{ {criteriaParams} }} -->
    <v-card :to="{ name: 'carrier:shipment:index', query: { status, ...nCriteria } }" flat>
        <EntityPageCount :fetcher="() => countShipments({ criteria: { ...criteria, status, filter } })"
            :uri="`/api/carrier/shipment/shipments/count?status=${status}&${criteriaParams}`">
...
import { getPaginatedShipments, countShipments as _countShipments } from '@/carrier/repository/shipment/shipment_repository';
