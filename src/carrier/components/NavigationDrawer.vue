<template>
    <v-app-bar flat>
                ...
                <template v-slot:append>
                    <v-menu>
                        <template v-slot:activator="{ props }">
                            <v-btn v-bind="props" :elevation="0" variant="plain">
        ...
        <v-toolbar-items v-if="!xs">
            ...
            <v-list-item prepend-icon="mdi-email" title="Email Templates" :to="{ name: 'carrier:mailing:template:index' }"
                value="email_templates"></v-list-item>
            ...
            <v-spacer />
            <v-list-item @click="() => toggleColorScheme()" nav>
                <template v-slot:prepend>
    ...
    <!-- <v-bottom-navigation r-v-model="value" r-bg-color="color" mode="shift">
        ...
        <v-btn :to="{ name: 'carrier:shipment:index' }">
            <v-icon>mdi-package-variant-closed</v-icon>
            <span>Shipments</span>
        </v-btn>

        <v-btn :to="{ name: 'carrier:mailing:template:index' }">
            <v-icon>mdi-email</v-icon>
            <span>Templates</span>
        </v-btn>

        <v-btn :to="{ name: 'admin:home' }" v-if="isGranted(['ROLE_ADMIN'])">
            <v-icon>mdi-account-outline</v-icon>
            <span>Admin</span>
        </v-btn>

        <v-btn @click.stop="() => toggleColorScheme()">
            <template v-if="isDark">
                <i class="v-icon notranslate v-theme--light v-icon--size-default v-icon--start" aria-hidden="true"><svg
            ...
            <v-responsive class="align-center fill-height pa-0 ma-0">
                ...
                <v-list density="compact" nav>
                    ...
                    <v-list-item prepend-icon="mdi-email" title="Email Templates"
                        :to="{ name: 'carrier:mailing:template:index' }" value="email_templates"></v-list-item>
...
const { isGranted, logout: doLogout } = useAccountStore();
