<template>
    ...
    <v-menu :close-on-content-click="false">
        <template v-slot:activator="{ props }">
            <v-text-field v-model="data.expiryDate" v-bind="props" label="Expiry Date"
                :placeholder="new Date().toLocaleDateString()" variant="outlined" density="compact" readonly />
        </template>
        <v-date-picker v-model="data.expiryDate" label="Expiry Date" variant="outlined" density="compact"></v-date-picker>
    </v-menu>

    <v-select v-model="data.timeFrameType" :items="deliveryTypes" label="Delivery Type" variant="outlined"
...
const deliveryTypes = ["REGULAR", "EXPRESS", "SAME_DAY"];
</script>