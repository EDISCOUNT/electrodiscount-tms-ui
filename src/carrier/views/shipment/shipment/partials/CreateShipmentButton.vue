<template>
    <v-menu>
        <template v-slot:activator="{ props }">
            <slot name="activator" v-bind="{ props, to: { name: 'carrier:shipment:index' } }">
        ...
        <v-card flat>
            <v-card-text>
                ...
                <v-card-actions>
                    <v-btn :to="{ name: 'carrier:shipment:create' }" color="primary" variant="flat" :elevation="0" block>
                ...
                <v-card-text class="pa-0">
                    ...
                    <v-list v-if="pagination">
                        <div v-for="(channel, i) in pagination.items" :key="channel.code ?? channel.id ?? i">
                            <v-list-item :to="{ name: 'carrier:shipment:create', }">
